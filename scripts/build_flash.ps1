...
$uvprojx = Get-ChildItem "$ProjectDir\Projects\MDK-ARM\*.uvprojx" -ErrorAction SilentlyContinue |
    Select-Object -First 1 -ExpandProperty FullName
if (-not $uvprojx) {
    $uvprojx = Get-ChildItem "$ProjectDir\MDK-ARM\*.uvprojx" -ErrorAction SilentlyContinue |
        Select-Object -First 1 -ExpandProperty FullName
}
if (-not $uvprojx) {
    $uvprojx = Get-ChildItem "$ProjectDir\*.uvprojx" -Recurse -ErrorAction SilentlyContinue |
        Select-Object -First 1 -ExpandProperty FullName
}
$isKeil = [bool]$uvprojx
...
if (-not $SkipBuild) {
    ...
    if ($isKeil) {
        ...
        $keilDir = Split-Path $uvprojx -Parent
        $buildLog = Join-Path $keilDir "build_log.txt"
        ...
        $elf = Get-ChildItem "$ProjectDir\Output\*.axf" -ErrorAction SilentlyContinue |
            Select-Object -First 1 -ExpandProperty FullName
        if (-not $elf) {
            $elf = Get-ChildItem "$ProjectDir\MDK-ARM\*\*.axf" -ErrorAction SilentlyContinue |
                Select-Object -First 1 -ExpandProperty FullName
        }
        if (-not $elf) {
            $elf = Get-ChildItem "$ProjectDir" -Filter "*.axf" -Recurse -ErrorAction SilentlyContinue |
                Select-Object -First 1 -ExpandProperty FullName
        }
